...
my($opt, $usage) = describe_options("%c %o < input",
				    ["allow-rebuild" => "Allow a rebuild of kmers to occur"],
				    ["data-dir|d=s" => "kmer data directory"],
				    ["url|u=s" => "kmer server URL"],
				    ["output|o=s" => "Write output to given file instead of stdout"],
...
if ($opt->allow_rebuild && $dataD && ! -s "$dataD/final.kmers")
{
    my $which_seed;
    if ($use_pub_seed)
    {
...
if ($dataD && ! -s "$dataD/kmer.table.mem_map")
{
    ...
    {
	die "$dataD/kmer.table.mem_map is missing and we are not rebuilding\n";
    }
